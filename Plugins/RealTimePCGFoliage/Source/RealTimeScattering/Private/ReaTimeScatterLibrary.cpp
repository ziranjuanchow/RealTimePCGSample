...
void FPointCloudReadBackBuffer::InitBuffer(int32 InMaxNum)
{
	...
	FRHIResourceCreateInfo CreateInfo(*"RealTimePCG");
...
void FPointCloudReadBackBuffer::ReadBackToArray(TArray<FScatterPoint>& OutputData)
{
	...
	uint32 Count = *(uint32*)RHILockBuffer(InstanceCountBuffer, 0, sizeof(uint32), EResourceLockMode::RLM_ReadOnly);
	RHIUnlockBuffer(InstanceCountBuffer.GetReference());
	FScatterPoint* Srcptr = (FScatterPoint*)RHILockBuffer(OutputBuffer, 0, sizeof(FScatterPoint) * MaxNum, EResourceLockMode::RLM_ReadOnly);
	...
	RHIUnlockBuffer(OutputBuffer.GetReference());
...
)
{
	...
	FVector2f TotalSize = FVector2f(TotalRect.Z, TotalRect.W) - FVector2f(TotalRect.X, TotalRect.Y);

	FIntPoint TexSize = DensityTexture->GetSizeXY();
	FVector2f ScaledPatternSize = Pattern.Size * RadiusScale;
	FVector2f Temp = TotalSize / ScaledPatternSize;
	int PerPointsCnt = Pattern.PointCloud.Num();
	FIntPoint CellNumber = FIntPoint(ceilf(Temp.X), ceilf(Temp.Y));

	
	Temp = (FVector2f(SimulateRect.X, SimulateRect.Y) - FVector2f(TotalRect.X, TotalRect.Y)) / ScaledPatternSize;
	FIntPoint StartCell = FIntPoint(floorf(Temp.X), floorf(Temp.Y));
	
	
	
	Temp =  (FVector2f(SimulateRect.Z, SimulateRect.W) - FVector2f(TotalRect.X, TotalRect.Y)) / ScaledPatternSize;
	...
	for (int i = 0; i < Pattern.PointCloud.Num() -1 ;)
	{
		Params.InputPattern[i][0] = Pattern.PointCloud[i].X;
		Params.InputPattern[i][1] = Pattern.PointCloud[i].Y;
		i += 2;
	}
	...
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	FVector2f TotalRectSize = FVector2f(TotalRect.Z - TotalRect.X, TotalRect.W - TotalRect.Y);
	
	FVector2f Temp;

	Temp = (FVector2f(SimulateRect.X, SimulateRect.Y) - FVector2f(TotalRect.X, TotalRect.Y)) / TotalRectSize;
	Temp *= InputSeedTexture->GetSizeXY();
	Temp /= GroupSize;
	FIntPoint StartGroup =  FIntPoint(floorf(Temp.X), floorf(Temp.Y));

	Temp = (FVector2f(SimulateRect.Z, SimulateRect.W) - FVector2f(TotalRect.X, TotalRect.Y)) / TotalRectSize;
	Temp *= InputSeedTexture->GetSizeXY();
	Temp /= GroupSize;
	...
	FRHITextureCreateDesc CreateSdfSeedDesc = 
		FRHITextureCreateDesc::Create2D(TEXT("RealTimePCG"), TexSize.X, TexSize.Y, PF_R32_FLOAT)
			.SetNumMips(1)
			.SetNumSamples(1)
			.SetFlags(Flags);
	FTexture2DRHIRef PingPointRT1 = RHICreateTexture(CreateSdfSeedDesc);
	FUnorderedAccessViewRHIRef PingPointRT1UAV = RHICreateUnorderedAccessView(PingPointRT1);
	FTexture2DRHIRef PingPointRT2 = RHICreateTexture(CreateSdfSeedDesc);
...
)
{
	...
	FRHITextureCreateDesc CreateSdfSeedDesc = 
		FRHITextureCreateDesc::Create2D(TEXT("RealTimePCG"), TexSize.X, TexSize.Y, PF_R32_FLOAT)
			...
			.SetNumSamples(1)
			.SetFlags(Flags);
	FTexture2DRHIRef SDFSeedTexture = RHICreateTexture(CreateSdfSeedDesc);
	FUnorderedAccessViewRHIRef SDFSeedTextureUAV = RHICreateUnorderedAccessView(SDFSeedTexture);
	FTexture2DRHIRef OutputSDFRT = RHICreateTexture(CreateSdfSeedDesc);
	FUnorderedAccessViewRHIRef OutputSDFRTUAV = RHICreateUnorderedAccessView(OutputSDFRT);
	FTexture2DRHIRef PlacementSDFRT = RHICreateTexture(CreateSdfSeedDesc);
	...
	FVector4f SimulateRect = Context.DirtyRect + FVector4f(-MaxSpeciesRadiusScale, -MaxSpeciesRadiusScale, MaxSpeciesRadiusScale, MaxSpeciesRadiusScale) * 100;
